(() => {
  ...
  if (!root) return;
  const REGISTRY_KEY = 'tile_tools_projects_v1';
  ...
  function collectLocalProjects() {
    const existing = new Map(readRegistry().map((item) => [String(item.id), normalize(item)]));
    const upsertSnapshot = (record) => { const previous = existing.get(record.id); existing.set(record.id, normalize({ ...previous, ...record, status: previous?.status || record.status, favorite: previous?.favorite || false })); };
    const calculator = safeParse(localStorage.getItem('poschitay-plitku.project.v1'));
    if (calculator?.id && calculator?.room) upsertSnapshot({ id: localId('calculation', calculator.id), source: 'local', type: 'calculation', title: calculator.name || 'Расчёт плитки', status: 'active', payload: calculator, updatedAt: calculator.updatedAt });
    const currentPdf = safeParse(localStorage.getItem('plitka_pdf_current_project'));
    if (currentPdf?.id && Array.isArray(currentPdf.pages)) upsertSnapshot({ id: localId('pdf', currentPdf.id), source: 'local', type: 'pdf', title: currentPdf.title || 'PDF-документ', status: 'active', payload: currentPdf, updatedAt: currentPdf.updatedAt });
    const savedPdf = safeParse(localStorage.getItem('plitka_pdf_saved_projects'), []) || [];
    savedPdf.forEach((meta) => { const payload = safeParse(localStorage.getItem(`plitka_pdf_saved_projects_${meta.id}`)); if (payload) upsertSnapshot({ id: localId('pdf', meta.id), source: 'local', type: 'pdf', title: meta.title || payload.title, status: 'draft', payload, updatedAt: meta.updatedAt || payload.updatedAt }); });
    const result = [...existing.values()]; writeRegistry(result); return result;
  }
  async function loadProjects() {
    const local = collectLocalProjects(); let server = [];
    try { const response = await fetch('/api/projects/'); if (response.ok) { const data = await response.json(); server = (data.projects || []).map(normalize); } } catch (_) {}
    const linkedServerIds = new Set(local.map((item) => item.serverId).filter(Boolean));
    const merged = new Map(server.filter((item) => !linkedServerIds.has(item.id)).map((item) => [item.id, item])); local.forEach((item) => merged.set(item.id, item)); projects = [...merged.values()]; render();
  }
  ...
  async function resume(project) {
    ...
    if (project.type === 'calculation' && payload) localStorage.setItem('poschitay-plitku.project.v1', JSON.stringify(payload));
    if (project.type === 'pdf' && payload) localStorage.setItem('plitka_pdf_current_project', JSON.stringify(payload));
    if (project.type === 'visualization' && payload) sessionStorage.setItem(`tile_tools_resume_${project.id}`, JSON.stringify(payload));
  ...
  root.addEventListener('click', (event) => {
    ...
    if (project && event.target.closest('[data-project-resume]')) { void resume(project); return; } if (project && event.target.closest('[data-project-favorite]')) { project.favorite = !project.favorite; saveLocalChange(project); render(); return; } if (project && event.target.closest('[data-project-archive]')) void updateStatus(project, project.status === 'archived' ? 'active' : 'archived');
  });
