(() => {
  ...
  function filteredItems() {
    const query = $('media-search').value.trim().toLocaleLowerCase('ru'); const filters = { brand:$('media-brand').value, colors:$('media-color').value, sizes:$('media-size').value, surfaces:$('media-surface').value, designs:$('media-design').value };
    let items = state.items.filter((item) => { if (query && !`${item.name} ${item.shortName || ''} ${item.brand || ''}`.toLocaleLowerCase('ru').includes(query)) return false; return Object.entries(filters).every(([key,value]) => !value || (key === 'brand' ? item.brand === value : (item[key] || []).includes(value))); });
    if (state.activeFolder === 'favorites') items = items.filter((item) => state.favorites.has(item.id)); else if (state.activeFolder !== 'all') items = items.filter((item) => (state.folderTiles[state.activeFolder] || []).includes(item.id));
    return items.sort((a,b) => Number(state.favorites.has(b.id)) - Number(state.favorites.has(a.id)) || (a.shortName || a.name).localeCompare(b.shortName || b.name, 'ru'));
  }
  function render() {
    const items = filteredItems(); $('media-total').textContent = `${state.items.length} плиток в общем каталоге`; $('media-all-count').textContent = state.items.length; $('media-favorite-count').textContent = state.favorites.size; $('media-result-count').textContent = items.length;
    $('media-active-caption').textContent = state.activeFolder === 'all' ? 'Все материалы' : state.activeFolder === 'favorites' ? 'Избранное' : state.folders.find((f) => f.id === state.activeFolder)?.name || 'Папка'; document.querySelectorAll('.media-nav-button').forEach((button) => button.classList.toggle('is-active', button.dataset.folder === state.activeFolder));
    if (!items.length) { $('media-grid').innerHTML = '<div class="media-empty"><strong>Ничего не найдено</strong><p>Измените фильтры или выберите другой раздел.</p></div>'; return; }
