...
function analytics_track_event($userId, $sessionKey, $eventName, $payload = []) {
    ...
    // events_count растёт на каждом событии; pageviews — только на page_view
    // (раньше pageviews не инкрементировался вовсе и всегда оставался 0).
    db()->prepare('UPDATE analytics_sessions SET events_count = events_count + 1, pageviews = pageviews + ?, last_seen_at = NOW() WHERE session_key = ?')
        ->execute([$eventName === 'page_view' ? 1 : 0, $sessionKey]);
    return true;
}
